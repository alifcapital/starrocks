...
// ---------------------------------------------------------------------------
// NGRAM bloom filter writer/reader pairing for UTF-8: under case_insensitive,
// non-ASCII rows must be lowered via utf8_tolower (StringZilla), ASCII rows via
// Slice::tolower. The reader's pushdown helper mirrors the same split so the
// pair stays bit-identical.
// ---------------------------------------------------------------------------
...
TEST_F(BloomFilterIndexReaderWriterTest, test_ngram_utf8_case_insensitive) {
    ...
    // "ПРИВЕТ" -> utf8_tolower -> "привет" -> character trigrams
