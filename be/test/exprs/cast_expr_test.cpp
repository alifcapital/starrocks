...
TEST_F(DatetimeCastReuseTest, ConjunctScopeAndShortCircuit) {
    ...
    Filter filter(2, 1);
    auto rows = ChunkPredicateEvaluator::eval_conjuncts_into_filter({lower, upper}, &chunk, &filter);
    ASSERT_TRUE(rows.ok());
    ...
    filter.assign(2, 1);
    rows = ChunkPredicateEvaluator::eval_conjuncts_into_filter({lower, strict_other}, &chunk, &filter);
    ASSERT_TRUE(rows.ok());
