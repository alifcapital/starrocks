...
// ---------------------------------------------------------------------------
// UTF-8 path: non-ASCII characters are treated as units, lowercase conversion via
...
TEST_F(NgramSearchTest, GermanSharpS_Insensitive) {
    // Lowercase preserves sharp-s.
    std::unique_ptr<RuntimeState> state;
    auto ctx = make_ctx(&state);
    auto columns = build_columns("STRAẞE", "straße", 3);
...
TEST_F(NgramSearchTest, LowercaseDoesNotFoldSharpS) {
    std::unique_ptr<RuntimeState> state;
    auto ctx = make_ctx(&state);
    auto columns = build_columns("SS", "ß", 1);
    EXPECT_DOUBLE_EQ(0.0, eval(ctx.get(), columns, CASE_INSENSITIVE));
}

TEST_F(NgramSearchTest, RestoresZeroHashAcrossRowsAndConstChunks) {
    std::string needle;
    for (int i = 0; i < (1 << 21); ++i) {
        std::string candidate = {char(i & 127), char((i >> 7) & 127), char((i >> 14) & 127)};
        if ((crc_hash_32(candidate.data(), candidate.size(), CRC_HASH_SEEDS::CRC_HASH_SEED1) & 0xffffu) == 0) {
            needle = candidate;
            break;
        }
    }
    ASSERT_EQ(3, needle.size());
    auto ctx = std::unique_ptr<FunctionContext>(FunctionContext::create_test_context());
    auto pattern = ColumnHelper::create_const_column<TYPE_VARCHAR>(needle, 2);
    auto gram = ColumnHelper::create_const_column<TYPE_INT>(3, 2);
    ctx->set_constant_columns({nullptr, pattern, gram});
    ASSERT_OK(StringFunctions::ngram_search_prepare(ctx.get(), FunctionContext::FRAGMENT_LOCAL));
    auto haystack = BinaryColumn::create();
    haystack->append(needle);
    haystack->append(needle);
    for (int repeat = 0; repeat < 3; ++repeat) {
        auto result = StringFunctions::ngram_search(ctx.get(), {haystack, pattern, gram});
        ASSERT_OK(result.status());
        EXPECT_DOUBLE_EQ(1.0, result.value()->get(0).get_double());
        EXPECT_DOUBLE_EQ(1.0, result.value()->get(1).get_double());
        result = StringFunctions::ngram_search(ctx.get(), {pattern, pattern, gram});
        ASSERT_OK(result.status());
        EXPECT_DOUBLE_EQ(1.0, result.value()->get(0).get_double());
    }
    ASSERT_OK(StringFunctions::ngram_search_close(ctx.get(), FunctionContext::FRAGMENT_LOCAL));
