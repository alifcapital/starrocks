...
// ---------------------------------------------------------------------------
// ngram_bloom_filter pushdown helper: validates the needle, lowers it via
// the same ASCII/UTF-8 split as the writer, then probes the bloom filter.
// Covers the index reader path for ngram_bf with UTF-8 needles.
// ---------------------------------------------------------------------------
...
TEST_F(NgramBloomFilterPushdownTest, InvalidUtf8NeedleDisablesIndex) {
    ...
    // returns true so the scanner falls back to full scan rather than filtering
    // pages based on a malformed needle.
    EXPECT_TRUE(expr.ngram_bloom_filter(&exprContext, bf.get(), opts));
