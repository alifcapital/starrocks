...
TEST_F(SharedBufferedInputStreamTest, test_parallel_read_concurrent) {
    ...
    for (size_t i = 0; i < 4 * MB; ++i) {
        ASSERT_EQ(data[4 * MB + i], static_cast<char>(buffer[i])) << "Mismatch at offset " << (4 * MB + i);
    }
    // One read_at_fully per chunk; each 4MB range is a single chunk (≤ 8MB * 1.5).
...
// Test: 8 parallel ranges all read; each fires read_at_fully once.
TEST_F(SharedBufferedInputStreamTest, test_parallel_read_sliding_window) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        for (size_t j = 0; j < 4 * MB; ++j) {
            ASSERT_EQ(data[i * 4 * MB + j], static_cast<char>(buffer[j]));
        }
    }
    // 8 ranges, each ≤ 8MB * 1.5 so single chunk each.
