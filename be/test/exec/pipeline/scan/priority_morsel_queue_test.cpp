...
// The 4.1 factory checks the queue directly before assigning morsels to drivers.
TEST(PriorityMorselQueueTest, forces_shared_queue) {
    Morsels morsels;
    morsels.emplace_back(bound_morsel(10, 15));
    PriorityMorselQueue queue(std::move(morsels), false, kSlotId, false, false);
    queue.set_max_degree_of_parallelism(8);
    EXPECT_FALSE(queue.can_uniform_distribute());
