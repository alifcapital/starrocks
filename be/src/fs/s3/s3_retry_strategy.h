...
// Enhanced S3 retry strategy based on ClickHouse's implementation.
// Features:
...
public:
    ...
    // Global throttling support - call before each S3 request
    static void wait_for_global_throttle();
...
private:
    ...
    void update_global_throttle_time(uint64_t delay_ms) const;

    Config _config;

    // Global throttle time (shared across all S3 clients)
