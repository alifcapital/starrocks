...
FooterWarmResult warm_footer(const pipeline::FooterPrefetchItem& item, bool metacache_on) {
    ...
    // FileMetaDataParser reads case_sensitive + split_context from the scan context and writes its
    // footer counters into ctx.stats. No split (warm always opens the root file); case sensitivity
    // comes from the shared open context.
    HdfsScannerContext ctx;
    ctx.options.case_sensitive = octx->case_sensitive;
    ctx.stats = &stats;
