...
inline uint32_t length_eq_mask(const uint32_t* offsets, size_t base, uint32_t target_len) {
    ...
    return (mask_arr[0] ? 1u : 0u) | (mask_arr[1] ? 2u : 0u) | (mask_arr[2] ? 4u : 0u) |
           (mask_arr[3] ? 8u : 0u);
#else
...
inline uint32_t length_in_range_mask(const uint32_t* offsets, size_t base, uint32_t min_len, uint32_t max_len) {
    ...
    __m256i off_next = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(offsets + base + 1));
    __m256i lengths = _mm256_sub_epi32(off_next, off_curr);
    __m256i min_vec = _mm256_set1_epi32(min_len);
    __m256i max_vec = _mm256_set1_epi32(max_len);
    __m256i ge_min = _mm256_cmpgt_epi32(lengths, _mm256_sub_epi32(min_vec, _mm256_set1_epi32(1)));
    __m256i le_max = _mm256_cmpgt_epi32(_mm256_add_epi32(max_vec, _mm256_set1_epi32(1)), lengths);
