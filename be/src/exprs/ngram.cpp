...
public:
    ...
    StatusOr<ColumnPtr> static ngram_search_impl(FunctionContext* context, const Inputs& columns) {
        ...
        // (e.g. too few characters), so similarity is 0.
    ...
    Status static ngram_search_prepare_impl(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
        ...
        int gram_num = ColumnHelper::get_const_value<TYPE_INT>(gram_num_column);
        if (gram_num <= 0) return Status::OK();
...
private:
    ...
    // Full Unicode lowercase uses the same mapping as SQL LOWER.
    static void tolower_utf8(const Slice& str, std::string& buf) { utf8_tolower(str.get_data(), str.get_size(), buf); }
    ...
    ColumnPtr static haystack_and_needle_non_const(const Inputs& columns, size_t gram_num) {
        ...
        for (size_t i = 0; i < chunk_size; i++) {
            ...
            if (same_needle) {
                ...
                if (cur_needle_slice.get_size() >= gram_num && raw_needle.get_size() <= MAX_STRING_SIZE) {
                    needle_gram_count =
            ...
            if (raw_haystack.get_size() > MAX_STRING_SIZE || needle_gram_count == 0) {
                res->get_data()[i] = 0;
                continue;
            }
    ...
    ColumnPtr static haystack_vector_and_needle_const(const ColumnPtr& haystack_column, std::vector<NgramHash>& map,
        ...
        map_restore_helper.reserve(MAX_STRING_SIZE);
        ...
        for (size_t i = 0; i + gram_num <= num_chars; i++) {
            ...
            if (map[cur_hash] > 0) {
                ...
                if constexpr (need_recovery_map) {
                    map_restore_helper.push_back(cur_hash);
                }
            }
        }

        if constexpr (need_recovery_map) {
            for (auto hash : map_restore_helper) map[hash]++;
            map_restore_helper.clear();
        }
...
StatusOr<ColumnPtr> StringFunctions::ngram_search(FunctionContext* context, const Columns& columns) {
    return NgramFunctionImpl<false>::ngram_search_impl(context, columns);
}
...
StatusOr<ColumnPtr> StringFunctions::ngram_search_case_insensitive(FunctionContext* context, const Columns& columns) {
    return NgramFunctionImpl<true>::ngram_search_impl(context, columns);
}
...
Status StringFunctions::ngram_search_prepare(FunctionContext* context, FunctionContext::FunctionStateScope scope) {
    return NgramFunctionImpl<false>::ngram_search_prepare_impl(context, scope);
}

Status StringFunctions::ngram_search_case_insensitive_prepare(FunctionContext* context,
                                                              FunctionContext::FunctionStateScope scope) {
    return NgramFunctionImpl<true>::ngram_search_prepare_impl(context, scope);
}
