...
private:
    // Byte offsets of the start of each UTF-8 character in [data, data+len).
    ...
    size_t static calculateMapWithNeedle(std::vector<NgramHash>& map, const Slice& needle, size_t gram_num) {
        ...
        if constexpr (case_insensitive) {
            utf8_tolower(needle.get_data(), needle.get_size(), buf);
    ...
    ColumnPtr static haystack_vector_and_needle_const(const ColumnPtr& haystack_column, std::vector<NgramHash>& map,
        ...
        // Case folding runs per-row inside calculateDistanceWithHaystack so the byte
        // offsets used here line up with the same code path that built the needle map.
