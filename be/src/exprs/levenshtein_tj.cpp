...
// Input is validated before decoding, so each codepoint is contained in this slice.
...
static double weighted_distance(const std::vector<Codepoint>& a, const std::vector<Codepoint>& b) {
    // Symmetric costs let us keep the shorter side in the DP rows.
    if (b.size() > a.size()) return weighted_distance(b, a);
    size_t m = a.size();
    size_t n = b.size();
    if (m == 0) {
        double s = 0;
...
StatusOr<ColumnPtr> StringFunctions::levenshtein_tj_distance(FunctionContext* context, const Columns& columns) {
    ...
    for (size_t i = 0; i < num_rows; i++) {
        ...
        if (!validate_utf8(s1.data, s1.size) || !validate_utf8(s2.data, s2.size)) {
            return Status::InvalidArgument("Tajik Levenshtein functions require valid UTF-8 input");
        }
...
StatusOr<ColumnPtr> StringFunctions::levenshtein_tj_ratio(FunctionContext* context, const Columns& columns) {
    ...
    for (size_t i = 0; i < num_rows; i++) {
        ...
        // Preserve the original calibrated score and its 0.9 thresholds.
        // This is intentionally twice the penalty of ordinary Indel similarity.
