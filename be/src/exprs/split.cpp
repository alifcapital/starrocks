...
StatusOr<ColumnPtr> StringFunctions::split_impl(FunctionContext* context, const Inputs& columns) {
    ...
    FunctionColumnViewer<TYPE_VARCHAR, Inputs> string_viewer = FunctionColumnViewer<TYPE_VARCHAR, Inputs>(columns[0]);
    FunctionColumnViewer<TYPE_VARCHAR, Inputs> delimiter_viewer =
            FunctionColumnViewer<TYPE_VARCHAR, Inputs>(columns[1]);

    size_t row_nums = input_num_rows(columns);
    ...
    //Array Binary
    const auto* haystack_columns =
            down_cast<const BinaryColumn*>(ColumnHelper::get_data_column(input_column(columns[0]).get()));
    BinaryColumn::MutablePtr array_binary_column = BinaryColumn::create();

    size_t selected_bytes = haystack_columns->get_immutable_bytes().size();
    if constexpr (std::is_same_v<Inputs, SelectedColumns>) {
        selected_bytes = 0;
        for (size_t row = 0; row < row_nums; ++row) selected_bytes += string_viewer.value(row).size;
    }
    ...
    if (context->is_notnull_constant_column(0) && context->is_notnull_constant_column(1)) {
        ...
        array_binary_column->reserve(row_nums * split_string.size(), selected_bytes);
    ...
    } else if (input_column(columns[1])->is_constant()) {
        Slice delimiter = state->delimiter;

        if (delimiter.size == 0) { // split each character
            std::vector<Slice> v;
            v.reserve(selected_bytes);
            array_binary_column->reserve(selected_bytes, selected_bytes);
            ...
            array_binary_column->reserve(row_nums * 5, selected_bytes);
            for (int row = 0; row < row_nums; ++row) {
                array_offsets->append(offset);
        ...
        if (!input_column(columns[0])->has_null()) {
            ...
            return NullableColumn::create(ArrayColumn::create(NullableColumn::create(std::move(array_binary_column),
                                                                                     NullColumn::create(offset, 0)),
                                                              std::move(array_offsets)),
                                          input_null_flags(columns[0], row_nums));
        }
    } else {
        array_binary_column->reserve(row_nums * 5, selected_bytes);
...
StatusOr<ColumnPtr> StringFunctions::split(FunctionContext* context, const Columns& columns) {
    return split_impl(context, columns);
}
StatusOr<ColumnPtr> StringFunctions::split_selected(FunctionContext* context, const SelectedColumns& columns, size_t) {
    return split_impl(context, columns);
}
