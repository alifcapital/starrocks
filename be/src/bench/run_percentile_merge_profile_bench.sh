#!/usr/bin/env bash
# Build + run percentile_merge_profile_bench standalone (no full BE / no JNI).
# Same shim approach as run_percentile_merge_scratch_bench.sh.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

CXX="${CXX:-g++}"
SHIM=be/src/bench/percentile_merge_scratch_shim
OUT=/tmp/percentile_merge_profile_bench
