...
// =====================================================================
//  count.h / sum.h :: get_values broadcast fill
// =====================================================================
//
// Pre-PR:
//   for (size_t i = start; i < end; ++i) data[i] = constant;
// Post-PR (AVX2 store4 / NEON store2 + scalar tail).
//
// Sweep over typical window-frame widths: 8 (small partition), 64
// (cache line), 256, 1024, 4096.  The crossover where SIMD wins over
...
#elif defined(__ARM_NEON) && defined(__aarch64__)
    const int64x2_t val_vec = vdupq_n_s64(value);
    size_t i = 0;
    for (; i + 2 <= count; i += 2) {
