...
bool FileReader::_evaluate_hashjoin_bloom_filters(const GroupReaderPtr& group_reader) {
    ...
    for (const auto& item : _scanner_ctx->runtime_filter_collector->descriptors()) {
        ...
        // Increment tried counter for hashjoin bloom filter intersection
        _scanner_ctx->stats->_optimzation_counter.hashjoin_bloom_intersection_tried_counter++;
        ...
        if (!any_partition_intersects) {
            // Successfully filtered using hashjoin bloom filter intersection
            _scanner_ctx->stats->_optimzation_counter.hashjoin_bloom_intersection_success_counter++;
            filtered = true;
            break;
        }
    }
