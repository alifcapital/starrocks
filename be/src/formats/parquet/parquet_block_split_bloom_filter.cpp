...
#ifdef __AVX2__
    // Vectorised hash test: AND the 8 mask lanes with the 8 block lanes; if any
    // lane is zero the hash cannot be present. cmpeq+movemask gives us a bitmask
    // of zero lanes; non-zero result means the hash is missing.
    __m256i v_block = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(block_offset));
    __m256i v_masks = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(masks));
