...
static inline size_t compare_integral_column_simd(CompareVector& cmp_vector, const T* lhs_data, T rhs_data,
    ...
    if (SIMD::count_zero(cmp_vector) > size / 8) {
        ...
        if constexpr (sizeof(T) == 1) {
            ...
            const __m256i sign_mask = std::is_unsigned_v<T> ? _mm256_set1_epi8(static_cast<char>(0x80))
            ...
            for (; i + kBlock <= size; i += kBlock) {
                __m256i cmp_bytes =
                        _mm256_loadu_si256(reinterpret_cast<const __m256i*>(cmp_vector.data() + i));
            ...
            const __m256i sign_mask = std::is_unsigned_v<T> ? _mm256_set1_epi16(static_cast<int16_t>(0x8000))
                                                            : _mm256_setzero_si256();
            // For unsigned compare, flip sign bit to reuse signed compares.
            if constexpr (std::is_unsigned_v<T>) {
                rhs_vec = _mm256_xor_si256(rhs_vec, sign_mask);
            ...
            for (; i + kBlock <= size; i += kBlock) {
                __m128i cmp_bytes =
                        _mm_loadu_si128(reinterpret_cast<const __m128i*>(cmp_vector.data() + i));
            ...
            const __m256i sign_mask = std::is_unsigned_v<T> ? _mm256_set1_epi32(0x8000'0000)
                                                            : _mm256_setzero_si256();
            // For unsigned compare, flip sign bit to reuse signed compares.
            if constexpr (std::is_unsigned_v<T>) {
                rhs_vec = _mm256_xor_si256(rhs_vec, sign_mask);
            ...
            const __m256i sign_mask =
                    std::is_unsigned_v<T> ? _mm256_set1_epi64x(static_cast<int64_t>(0x8000'0000'0000'0000ULL))
                                          : _mm256_setzero_si256();
            // For unsigned compare, flip sign bit to reuse signed compares.
            if constexpr (std::is_unsigned_v<T>) {
                rhs_vec = _mm256_xor_si256(rhs_vec, sign_mask);
        ...
        if constexpr (sizeof(T) == 1) {
            ...
            const uint64x2_t sign_mask = std::is_unsigned_v<T> ? vdupq_n_u64(0x8000'0000'0000'0000ULL)
                                                               : vdupq_n_u64(0);
            // For unsigned compare, flip sign bit to reuse signed compares.
            if constexpr (std::is_unsigned_v<T>) {
