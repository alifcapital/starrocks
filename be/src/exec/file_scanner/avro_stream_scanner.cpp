...
Status AvroStreamScanner::fill_row(const avro::GenericDatum& datum, Chunk* chunk) {
    ...
    for (size_t i = 0; i < _src_slot_descriptors.size(); ++i) {
        ...
        if (use_jsonpath) {
            ...
            if (record->hasField(slot_desc->col_name())) {
                RETURN_IF_ERROR(_column_readers[i]->read_datum_for_adaptive_column(record->field(slot_desc->col_name()),
                                                                                   column));
            } else {
                column->append_nulls(1);
            }
...
StatusOr<ChunkPtr> AvroStreamScanner::get_next() {
    ...
    if (src_chunk->num_rows() == 0) {
        ...
        // Timeout at the start of a chunk, or any other error: surface it.
        if (!st.ok()) {
            return st;
        }
    }
