...
StatusOr<const avro::GenericDatum*> avro_extract_field(const avro::GenericDatum& root,
    ...
    // Root is an array: only $[i] is valid. Don't peel a union first — a union-typed root is handled
    // by the descent loop below, not as a root array.
    ...
    for (size_t i = 1; i < path.size(); ++i) {
        ...
        if (type != avro::AVRO_RECORD && type != avro::AVRO_MAP) {
            if (i == path.size() - 1) {
                break; // reached a leaf earlier than the path expected; return it as-is
            }
            return Status::InternalError("a non-record/map type was found mid-path during avro parsing");
        }
