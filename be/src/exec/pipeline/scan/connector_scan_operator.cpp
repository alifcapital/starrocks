...
void ConnectorScanOperator::do_close(RuntimeState* state) {
    ...
    if (fp != nullptr && get_driver_sequence() == 0) {
        COUNTER_SET(ADD_COUNTER(_unique_metrics.get(), "FooterPrefetchPageCacheWrites", TUnit::UNIT), fp->warmed_pagecache());
        COUNTER_SET(ADD_COUNTER(_unique_metrics.get(), "FooterPrefetchBlockCacheWrites", TUnit::UNIT),
