...
const std::shared_ptr<spill::Spiller>& SpillablePartitionWiseAggregateSinkOperator::_spiller() const {
    return _agg_op->aggregator()->spiller();
}

SpillProcessChannelPtr SpillablePartitionWiseAggregateSinkOperator::_spill_channel() const {
    return _agg_op->aggregator()->spill_channel();
}
...
BlockReason SpillablePartitionWiseAggregateSinkOperator::block_reason() const {
...
Status SpillablePartitionWiseAggregateSinkOperator::prepare(RuntimeState* state) {
    ...
    // Propagate this wrapper's observer into the wrapped agg-op before its prepare(): the base
    // AggregateBlockingSinkOperator::prepare attaches the operator's _observer to the aggregator-pip
    ...
    // Subscribe this sink driver to the wrapped agg-op spiller's sink list so flush/channel completions wake
    // the OUTPUT_FULL sleeper. Unconditional: the poller-mode gate lives inside subscribe_sink.
    _agg_op->aggregator()->spiller()->observable().subscribe_sink(state, observer());
...
Status SpillablePartitionWiseAggregateSinkOperator::push_chunk(RuntimeState* state, const ChunkPtr& chunk) {
    ...
    if (_spill_strategy == spill::SpillStrategy::NO_SPILL) {
        ...
        set_revocable_mem_bytes(_agg_op->aggregator()->memory_usage());
        return Status::OK();
    }
...
Status SpillablePartitionWiseAggregateSinkOperator::_try_to_spill_by_auto(RuntimeState* state, const ChunkPtr& chunk) {
    ...
    const size_t ht_mem_usage = _agg_op->aggregator()->memory_usage();
    ...
    size_t revocable_mem_bytes = _streaming_bytes + _agg_op->aggregator()->memory_usage();
    ...
    if (revocable_mem_bytes > max_mem_usage) {
        ...
        bool should_spill_hash_table = _continuous_low_reduction_chunk_num >= HT_LOW_REDUCTION_CHUNK_LIMIT ||
                                       _agg_op->aggregator()->memory_usage() > max_mem_usage;
...
Status SpillablePartitionWiseAggregateSourceOperator::prepare(RuntimeState* state) {
    ...
    // AggregateBlockingSourceOperator::prepare attaches the operator's _observer to the aggregator-pip
    // observable, and assign_observer only set _observer on the wrapper. Without this the sub-op would
    // attach nullptr and notify_source would null-deref under the event scheduler (crash in release).
    // _pw_agg is a ConjugateOperator -- it re-propagates to its own sub-ops in its prepare(), but it still
    // needs a real observer of its own. Mirrors ConjugateOperator::prepare.
    _non_pw_agg->set_observer(observer());
    _pw_agg->set_observer(observer());
    ...
    _non_pw_agg->aggregator()->spiller()->observable().subscribe_source(state, observer());
    return Status::OK();
}

const std::shared_ptr<spill::Spiller>& SpillablePartitionWiseAggregateSourceOperator::_spiller() const {
    return _non_pw_agg->aggregator()->spiller();
}

bool SpillablePartitionWiseAggregateSourceOperator::_sink_complete() const {
    return _non_pw_agg->aggregator()->is_sink_complete();
}

bool SpillablePartitionWiseAggregateSourceOperator::_conjugate_finished() const {
    return _pw_agg->is_finished();
}
...
bool SpillablePartitionWiseAggregateSourceOperator::_non_pw_has_output() const {
    return _non_pw_agg->has_output();
}

bool SpillablePartitionWiseAggregateSourceOperator::_non_pw_finished() const {
