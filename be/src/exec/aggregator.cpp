    ...
    auto st = _hash_map_variant.visit([&](auto& variant_value) {
        ...
        if constexpr (std::is_integral_v<KeyType>) {
            ...
            while (it != end) {
                ...
                if (count > 100000) {
                    // TEMP DEBUG: dump the raw blob layout on heavy entries to locate the off-by-one.
                    LOG(WARNING) << "CCDBG collect key=" << static_cast<int64_t>(key) << " count=" << count
                                 << " count_offset=" << count_offset << " ks=" << _state_allocator.aggregate_key_size
                                 << " b0=" << *reinterpret_cast<const int64_t*>(value)
                                 << " b8=" << *reinterpret_cast<const int64_t*>(value + 8);
                }
