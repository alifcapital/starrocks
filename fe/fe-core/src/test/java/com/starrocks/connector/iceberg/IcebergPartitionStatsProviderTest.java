...
import static com.starrocks.connector.iceberg.IcebergCatalog.EMPTY_PARTITION_NAME;
...
public class IcebergPartitionStatsProviderTest extends TableTestBase {
    ...
    public void exactMatchStatsFile_returnsMap() throws Exception {
        ...
        PartitionStatisticsFile statsFile =
                PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableB, snapshotId);
    ...
    public void incrementalMerge_returnsMap() throws Exception {
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableB, statsSnapshotId);
    ...
    @Test
    public void bitEquivalenceVsPartitionsTable_unpartitioned() throws Exception {
        DataFile file = DataFiles.builder(PartitionSpec.unpartitioned())
                .withPath("/path/to/unpart.parquet")
                .withFileSizeInBytes(10)
                .withRecordCount(5)
                .build();
        mockedNativeTableH.newAppend().appendFile(file).commit();
        long snapshotId = mockedNativeTableH.currentSnapshot().snapshotId();
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableH, snapshotId);

        Optional<Map<String, Partition>> fast =
                IcebergPartitionStatsProvider.tryRead(mockedNativeTableH, snapshotId);
        Assertions.assertTrue(fast.isPresent(), "unpartitioned table with stats file must populate fast path");
        Assertions.assertEquals(1, fast.get().size());
        Assertions.assertTrue(fast.get().containsKey(EMPTY_PARTITION_NAME),
                "unpartitioned fast path must key on EMPTY_PARTITION_NAME, same as PartitionsTable path");

        IcebergTable wrapper = new IcebergTable(2, "srTable", "iceberg_catalog",
                "resource", "db", "th", "", Lists.newArrayList(), mockedNativeTableH, Maps.newHashMap());
    ...
    public void bitEquivalenceVsPartitionsTable_bucketTransform() throws Exception {
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableA, snapshotId);
    ...
    public void bitEquivalenceVsPartitionsTable_withDeleteFile() throws Exception {
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableC, snapshotId);
    ...
    public void rewriteManifests_doesNotChangeFingerprint() throws Exception {
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableB, beforeId);
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableB, afterId);
    ...
    public void expiredSnapshot_statsPathKeepsModifiedTime() throws Exception {
        ...
        PartitionStatsHandler.computeAndWriteStatsFile(mockedNativeTableB, s2);
