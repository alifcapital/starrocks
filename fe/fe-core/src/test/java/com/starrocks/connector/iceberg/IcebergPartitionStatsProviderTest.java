    ...
    private static void assertBitEquivalent(Map<String, Partition> reference, Map<String, Partition> fast) {
        ...
        for (Map.Entry<String, Partition> entry : reference.entrySet()) {
            ...
            Assertions.assertEquals(entry.getValue().getPositionDeleteRecordCount(), fastEntry.getPositionDeleteRecordCount(),
                    "position delete count must match for partition " + entry.getKey());
            Assertions.assertEquals(entry.getValue().getEqualityDeleteRecordCount(), fastEntry.getEqualityDeleteRecordCount(),
                    "equality delete count must match for partition " + entry.getKey());
        }
    }
}
