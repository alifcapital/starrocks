...
public class MergeTabletJobTest {
    ...
    public void testCleaningClearsPlacementPreferenceBeforeFinishing() throws Exception {
        ...
        try {
            ...
            // branch-4.1 only: this branch's cleaning path removes the superseded materialized
            // index from the partition outright, whereas main merely hands it to the recycle bin.
            // Every test in this class shares one static table and these are the only tests that
            // drive runCleaningJob() past the transaction wait, so the real removal would strip the
            // base index every later test resolves through getLatestBaseIndex(). Returning null also
            // makes the caller skip the matching inverted-index cleanup, leaving the fixture intact.
            // What this test is about -- that the pin is cleared from the real cleaning path, once,
            // before the job is marked FINISHED -- is unaffected.
