...
public class ExternalMultiColumnStatsCacheLoaderTest {
    @Test
    public void testParseGroup() {
        List<String> row = List.of("[\"status\",\"gate\"]", "1000", "12",
                "[[[\"approved\",\"0\"],\"540\",[\"700\",\"900\"]],[[null,\"1\"],\"7\"],[[\"short\"],\"3\"],"
                        + "[[\"zero\",\"0\"],\"0\"],[[\"odd\",\"2\"],\"5\",[\"9\"]]]");
        ExternalMultiColumnCombinedStatistics.Group group = ExternalMultiColumnStatsCacheLoader.parseGroup(row);
    ...
    public void testParseGroupToleratesMissingMcvAndRejectsBadRows() {
        ExternalMultiColumnCombinedStatistics.Group group = ExternalMultiColumnStatsCacheLoader.parseGroup(
                Arrays.asList("[\"a\",\"b\"]", "10", "3", null));
        Assertions.assertNotNull(group);
        Assertions.assertTrue(group.getMcv().isEmpty());

        Assertions.assertNull(ExternalMultiColumnStatsCacheLoader.parseGroup(Arrays.asList(null, "10", "3", "[]")));
        Assertions.assertNull(ExternalMultiColumnStatsCacheLoader.parseGroup(List.of("[\"a\"]", "10", "3", "[]")));
        Assertions.assertNull(ExternalMultiColumnStatsCacheLoader.parseGroup(List.of("not json", "10", "3", "[]")));
    }

    @Test
    public void testMetaGsonRoundTrip() {
        ExternalMultiColumnStatsMeta meta = new ExternalMultiColumnStatsMeta("hive0", "db", "t",
        ...
        ExternalMultiColumnStatsMeta copy = GsonUtils.GSON.fromJson(json, ExternalMultiColumnStatsMeta.class);
