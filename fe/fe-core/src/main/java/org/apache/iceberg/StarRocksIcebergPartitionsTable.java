...
/**
 * Iceberg 1.10 PARTITIONS aggregation with an explicit cache-aware manifest entry source.
 * Keep row layout, partition evolution and last_updated semantics aligned with PartitionsTable.
 */
public class StarRocksIcebergPartitionsTable extends BaseMetadataTable {

    private final Schema schema;
    private final StarRocksIcebergTableScan cachedScan;

    private final boolean unpartitionedTable;

    StarRocksIcebergPartitionsTable(Table table, StarRocksIcebergTableScan cachedScan) {
        this(table, table.name() + ".partitions", cachedScan);
    }

    StarRocksIcebergPartitionsTable(Table table, String name, StarRocksIcebergTableScan cachedScan) {
        super(table, name);
        this.cachedScan = cachedScan;
    ...
    private DataTask task(StaticTableScan scan) {
        Iterable<Partition> partitions = partitions(table(), scan, cachedScan);
        if (unpartitionedTable) {
            ...
            return StaticDataTask.of(
                    ...
                    scan.schema(),
                    partitions,
                    StarRocksIcebergPartitionsTable::convertPartition);
        }
    }
    ...
    private static Iterable<Partition> partitions(
            Table table, StaticTableScan scan, StarRocksIcebergTableScan cachedScan) {
        Types.StructType partitionType = Partitioning.partitionType(table);

        StructLikeMap<Partition> partitions =
                StructLikeMap.create(partitionType, new PartitionComparator(partitionType));

        try (CloseableIterable<ManifestEntry<? extends ContentFile<?>>> entries = planEntries(scan, cachedScan)) {
    ...
    @VisibleForTesting
    static CloseableIterable<ManifestEntry<?>> planEntries(StaticTableScan scan, StarRocksIcebergTableScan cachedScan) {
        ...
        Iterable<CloseableIterable<ManifestEntry<?>>> tasks =
                CloseableIterable.transform(filteredManifests, manifest -> readEntries(manifest, scan, cachedScan));

        return new ParallelIterable<>(tasks, scan.planExecutor());
    }

    private static CloseableIterable<ManifestEntry<?>> readEntries(
            ManifestFile manifest, StaticTableScan scan, StarRocksIcebergTableScan cachedScan) {
        return cachedScan.partitionEntries(manifest);
    }
    ...
    private class PartitionsScan extends StaticTableScan {
        PartitionsScan(Table table) {
            super(
                    table,
                    StarRocksIcebergPartitionsTable.this.schema(),
                    MetadataTableType.PARTITIONS,
                    StarRocksIcebergPartitionsTable.this::task);
        }
    }
