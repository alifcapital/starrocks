...
/**
 * Predicate preimages. The registry separates exact SQL equivalences from necessary scan
 * conditions for partial functions. The latter retain the original predicate at the scan;
 * they are never applied as scalar replacements inside SELECT, NOT or IS NULL.
 */
public final class MonotonicInverse {
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse PERIOD_FLOOR = MonotonicInverse::invertPeriodFloor;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse YEAR_PERIOD = MonotonicInverse::invertYear;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse DAY_FLOOR = MonotonicInverse::invertToDate;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse DATEDIFF_DAYS = MonotonicInverse::invertDatediff;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse RENDERED_PERIOD = MonotonicInverse::invertDateFormat;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse ISO_RENDER = MonotonicInverse::invertToIso8601;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse UNIX_EPOCH = MonotonicInverse::invertUnixTimestamp;
     ...
     * rendering can be NULL: the bounds are necessary scan conditions only, retaining the
     * original expression. Millisecond boundaries follow BE's division before range checking.
     * A period endpoint falling into a fall-back overlap of the session zone refuses: that
     * wall clock maps two epochs.
     */
    public static final MonotonicFunctionRegistry.PredicateInverse EPOCH_RENDER = MonotonicInverse::invertFromUnixTime;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse DAY_NUMBER = MonotonicInverse::invertToDays;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse UNIT_END = MonotonicInverse::invertLastDay;
     ...
     */
    public static final MonotonicFunctionRegistry.PredicateInverse DOW_NEXT = MonotonicInverse::invertNextDay;
    public static final MonotonicFunctionRegistry.PredicateInverse DOW_PREVIOUS = MonotonicInverse::invertPreviousDay;
    ...
    private static Optional<ScalarOperator> invertUnixTimestamp(CallOperator call, ScalarOperator dataChild,
                                                                BinaryType cmp, ConstantOperator value) {
        try {
            if (!call.getType().isBigint() || !dataChild.getType().isDatetime()
                    || !value.getType().isExactNumericType()) {
                return Optional.empty();
            }
            ...
            // Gap/overlap handling differs between Java and BE. A transition near the
            // comparison boundary can introduce another preimage even outside an overlap.
            if (!transitionFree(zone, lower.minusHours(26), upper.plusHours(26))) {
                return Optional.empty();
            }
            LocalDateTime maxExclusive = maxValid.plusSeconds(1);
            if (maxExclusive.isAfter(ConstantOperator.MAX_DATETIME)) {
                return Optional.empty();
            }
            switch (cmp) {
                case EQ:
                ...
                case GE:
                    return Optional.of(Utils.compoundAnd(
                            BinaryPredicateOperator.ge(dataChild, ConstantOperator.createDatetime(lower)),
                            BinaryPredicateOperator.lt(dataChild, ConstantOperator.createDatetime(maxExclusive))));
                case GT:
                    return Optional.of(Utils.compoundAnd(
    ...
    private static Optional<ScalarOperator> invertFromUnixTime(CallOperator call, ScalarOperator dataChild,
        ...
        try {
            ...
            // BE divides signed milliseconds before range checking, truncating toward zero.
            // -999..-1 therefore render epoch zero; the last valid second includes +999 ms.
            long minValid = 1 - unitScale;
            long maxValid = TimeUtils.MAX_UNIX_TIMESTAMP * unitScale + unitScale - 1;
            if (lower == 0) {
                lower = minValid;
            }
     ...
     * reject overflow rows. NE and null-safe equality refuse; the other comparisons supply
     * a necessary bound without imposing a second-resolution cap on DATETIME values.
     * Refuses when the opposite shift of the constant is invalid or cannot be folded.
     */
    public static MonotonicFunctionRegistry.PredicateInverse shift(String oppositeFnName) {
    ...
    private static Optional<ScalarOperator> invertShift(String oppositeFnName, CallOperator call,
        ...
        try {
            ...
            switch (cmp) {
                ...
                case LE:
                case LT:
                    // The original predicate removes overflow rows. A necessary bound need
                    // not reproduce that rejection, and avoids a precision-sensitive domain cap.
                    return Optional.of(inverted);
                default:
                    return Optional.empty();
            }
