...
public class DomainProperty {
    ...
    private DomainWrapper deriveMonotonicImageDomain(ColumnRefOperator outputColumn, ScalarOperator expr) {
        ...
        Range<ConstantOperator> image =
                MonotonicImage.imageRange(expr, column, columnWrapper.getMinMaxForMonotonicImage()).orElse(null);
        if (image == null) {
            return null;
        }
    ...
    public static class DomainWrapper {
        ...
        public MinMax getMinMaxForMonotonicImage() {
            ConnectContext context = ConnectContext.get();
            if (!needDeriveRange || context == null
                    || !context.getSessionVariable().isEnableStringDateJoinPruning()) {
                return minMax;
            }
            // Only the image evaluator may use these string bounds. It checks the canonical
            // date contract before folding; ordinary range derivation keeps its own policy.
            return deriveMinMax(predicateDesc, true);
        }
        ...
        private MinMax deriveMinMax(ScalarOperator scalarOperator) {
            return deriveMinMax(scalarOperator, false);
        }

        private MinMax deriveMinMax(ScalarOperator scalarOperator, boolean includeStringRanges) {
            RangeExtractor extractor = new RangeExtractor(includeStringRanges);
