...
public final class IcebergPartitionStatsProvider {
        ...
        try {
            ...
            if (plan.mode() == PartitionStatsPlan.Mode.INCREMENTAL) {
                PartitionMap<PartitionStats> delta = IcebergPartitionStatsHelper.computeStatsFromManifests(
                        nativeTable, plan.incrementalManifests(), partitionType, true /* incremental */);
    ...
    private static Optional<Map<String, Partition>> convertToPartitionMap(
        ...
        boolean unpartitioned = nativeTable.spec().isUnpartitioned();
        if (unpartitioned && statsMap.size() > 1) {
            // Defensive: if the table's current spec is unpartitioned but stats carry multiple
            // entries (e.g. legacy historical partitioned spec records), the unpartitioned branch
            // would silently collapse them onto EMPTY_PARTITION_NAME. Fail the fast path so the
            // existing PartitionsTable scan handles the table consistently.
            return Optional.empty();
        }
        ...
        for (PartitionStats stats : statsMap.values()) {
            ...
            if (unpartitioned) {
                partitionName = EMPTY_PARTITION_NAME;
            } else {
                partitionName = PartitionUtil.convertIcebergPartitionToPartitionName(
