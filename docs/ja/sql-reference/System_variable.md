...
* **デフォルト**: false
* **データ型**: Boolean
* **説明**: `string_date_predicate_format` で宣言した格納形式を使用し、`CAST(column AS DATE/DATETIME)` の比較に VARCHAR のスキャン範囲を追加します。対応する空でない形式の指定が必要です。通常の WHERE と JOIN から転送された述語の両方で利用でき、パーティションテーブルや `enable_string_date_join_pruning` は必須ではありません。

元の日付比較は保持します。ストレージが対応する場合、文字列範囲によりパーティション、ファイル、データブロックを除外できます。対応する日付関数では、まず `enable_monotonic_predicate_rewrite` が CAST の範囲を生成し、この設定が文字列列の範囲を生成します。DATE への CAST の等価条件は、時刻を含む文字列でも一日全体を対象とします。AND の正のフィルター条件のみを処理し、SELECT 式、OR、IS NULL 内の述語は置き換えません。

有効化は、**最適化に参加するすべての VARCHAR 日付列の非 NULL 値が有効な日付であり、指定した形式を使うこと**をユーザーが保証するものです。行の検証や CAST の解析動作の変更は行いません。参加する列間で形式が異なる場合は有効にしないでください。形式の不一致、短い年、ゼロ埋め不足、余分な空白、不正な値により、一致する行を除外する可能性があります。無関係のデータに対するグローバル設定より、セッションまたはクエリ単位での指定を推奨します。
...
各フィールドはゼロ埋めし、年は四桁、`%f` は小数点以下六桁に固定する必要があります。可変精度、文字列内の数値オフセットやゾーン名は未対応です。二つの `Z` 形式では FE JVM のシステムタイムゾーンが固定 UTC である必要があり、それ以外では境界を導出しません。パーサーを変更せずに FE の定数評価と BE の解析を一致させます。セッションタイムゾーンは異なっていても構いません。年より先に日や月を置く形式は、辞書順で日付範囲を表せないため受け付けません。
