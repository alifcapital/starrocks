...
`compression`: 整数値となる省略可能な定数式で、範囲は [100, 10000] です。評価結果が整数であれば、`5000.0` のような小数表記、キャスト、定数の算術式を使用できます。小数部分を持つ値や定数でない式はエラーになります。引数を省略した場合、または `NULL` を指定した場合は `1000` を使用します。`100` 未満の整数には `100`、`10000` を超える整数には `10000` を使用します。
