...
間隔や時間帯を変更すると、昼間でも次のスケジューラーの確認時に将来の予定を再計算します。実行中の収集は中断しません。現在時刻から再配置するため、前回の収集からの間隔が延びる場合があり、設定を繰り返し変更すると再び延期されることがあります。

詳細は [CBO の統計収集](../../../using_starrocks/Cost_based_optimizer.md) を参照してください。
