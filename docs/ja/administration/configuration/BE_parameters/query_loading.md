...
- 単位: -
- 変更可能: はい
- 説明: ハッシュ結合のプローブがソフトウェアプリフェッチをいつ有効にするかを、L2 キャッシュサイズに対する比率で制御します。結合のハッシュテーブルが `join_probe_prefetch_l2_ratio × L2` を超えて初めてプリフェッチが有効になります。小さいテーブルではプリフェッチがプローブを遅くするため無効になります。値を下げるとより小さいテーブルでプリフェッチが有効になり、上げるとより大きいテーブルに限定されます。`0` は常にプリフェッチします。デフォルト値 0.4 は、プリフェッチが正味でプラスになると初めて測定された境界です。
- 導入バージョン: -
