...
- 単位: 行
- 変更可能: はい
- 説明: 集計プローブとハッシュ結合プローブの両方で使用されるソフトウェアプリフェッチ距離（行単位）。プローブはこの行数だけ先のデータをプリフェッチし、大規模なハッシュテーブルでのメモリレイテンシを隠蔽します。`0` に設定するとソフトウェアプリフェッチが無効になります。デフォルト値の 16 はほとんどのワークロードに適しています。ハッシュテーブルが DRAM に溢れるほど大きい場合は値を大きくしてください。より長い先読みでより多くのレイテンシを隠蔽できます。値を小さくしても実質的な効果はありません。キャッシュに収まる小さいテーブルはすでに L2 比率によって除外されているためです。各パスはさらに独自の比率（`agg_prefetch_l2_ratio`、`join_probe_prefetch_l2_ratio`）で L2 レジデンシーに基づきゲートされます。
- 導入バージョン: -
...
- 単位: Bytes
- 変更可能: はい
- 説明: GROUP BY キーに NDV（カーディナリティ）推定値がある場合、集約オペレーターはハッシュテーブルを空から増分拡張するのではなく、その推定サイズで事前確保します。これにより、高カーディナリティの GROUP BY での繰り返しの rehash を回避します。本設定は単一の集約オペレーターが事前確保できる容量の上限です。`0` に設定すると事前確保を無効にします。
...
- 単位: -
- 変更可能: はい
- 説明: ハッシュ結合のプローブがソフトウェアプリフェッチをいつ有効にするかを、L2 キャッシュサイズに対する比率で制御します。結合のハッシュテーブルが `join_probe_prefetch_l2_ratio × L2` を超えて初めてプリフェッチが有効になります。小さいテーブルではプリフェッチがプローブを遅くするため無効になります。値を下げるとより小さいテーブルでプリフェッチが有効になり、上げるとより大きいテーブルに限定されます。`0` は常にプリフェッチします。デフォルト値 0.4 は、プリフェッチが正味でプラスになると初めて測定された境界です。負の値または非有限値を指定すると、プリフェッチは無効になります。
- 導入バージョン: -
