...
- 単位: カウント
- タイプ: 累積
- 説明: サンプリングベースのタブレット事前分割が駆動したサンプラー起動の累計回数。eligibility を通過した呼び出しごとに、本番サンプラーパイプラインがサンプル取得を開始した時点でインクリメントされます。
...
- 単位: カウント
- タイプ: ヒストグラム
- 説明: 1 回の起動でプランナーが生成した境界タプルの数。`effectiveTabletCount - 1` に等しい（K タブレットへの分割には K-1 個のカットポイントが必要）。

## `starrocks_fe_tablet_pre_split_partitions_total`

- 単位: カウント
- タイプ: 累積
- 説明: マルチパーティション経路（P2-a）のカウンタ。サンプリングベースのタブレット事前分割コーディネーターが集計した予測ターゲットパーティション数の合計 — grouper を通過した `PartitionSamples` エントリごとに 1 ずつインクリメント。`tablet_pre_split_partitions_capped` と `tablet_pre_split_pre_create{result=...}` 系統と合わせると、1 回のマルチパーティション起動が実際に何パーティションを扱ったかを把握できます。単一パーティション経路では 0 のままです。
...
- 単位: カウント
- タイプ: 累積
- ラベル: `result` — `succeeded`（`LocalMetastore.addPartitions` が正常終了 — パーティションが作成された、または黙って重複排除された）、`failed`（`addPartitions` が例外を送出。例：並行 ALTER、journal 書き込み失敗。該当パーティションは BE ランタイムの自動パーティション作成にフォールバックし、同時に `tablet_pre_split_sampler_failed{reason=pre_create_failed}` にも記録される）、`already_exists`（事前作成時にカタログ内にパーティションが既に存在する — 並行ローダー競合。コーディネーターは既存パーティションを再利用する）。
- 説明: マルチパーティション経路（P2-a）のカウンタ。コーディネーターが `LocalMetastore.addPartitions` 経由で発行したパーティション事前作成試行回数を、結果別に集計した累計値。試行総数 = 3 つのラベルの合計。単一パーティション経路では 0 のままです。

## `starrocks_fe_tablet_pre_split_pre_submit_wait_ms`

- 単位: ミリ秒
- タイプ: ヒストグラム
- 説明: サンプリングベースのタブレット事前分割の提出前フェーズ（サンプリング + プランニング + reshard ジョブの構築）に費やした壁時計時間。`tablet_pre_split_pre_submit_timeout_seconds` で上限が設定されます。
