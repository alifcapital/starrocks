...
| enable_statistic_auto_collect_staggered_schedule | BOOLEAN | FALSE | 既存・新規の自動収集をテーブル単位で間隔と日次ウィンドウ内に分散します。内部・外部テーブルとデータベース全体のジョブが対象です。 |
...
`enable_statistic_auto_collect_staggered_schedule=true` を設定すると、自動収集を**テーブル単位**で収集間隔と毎日の許可ウィンドウ内に分散します。デフォルトは `false` です。

例えば間隔が 604800 秒、ウィンドウが 01:00–05:00 の場合、各テーブルに今後七つの夜間ウィンドウ内の異なる曜日・時刻が割り当てられます。既存ジョブと新規ジョブ、内部・外部テーブル、データベース全体や全データベースのジョブが対象で、既存ジョブの再作成は不要です。

- 最初のスケジューラチェックで、収集を実行する前に時刻を割り当てます。週単位の場合、初回は今後一週間以内で、従来の期限にさらに一週間を加えません。有効化後の初回チェックは昼間でも既存ジョブの予定を初期化しますが、実際の収集は許可時間内に限定されます。
- ジョブ・テーブルの安定した識別子と間隔から時刻を決定します。収集成功時や収集不要と判断した場合、完了時刻から間隔を足すのではなく次のカレンダー上の時刻へ進めます。
- メタデータの再生後も将来の予定を保持します。再起動で逃した予定や閉じたウィンドウ内の期限は将来の時刻に再配置し、次の夜の開始時に一斉実行しません。週単位なら再配置先も今後一週間以内です。失敗した収集は同じ開いているウィンドウで再試行できます。
- 自動収集器と同じタイムゾーン・日次ウィンドウを使用し、日付をまたぐ設定も扱います。終了前にチェック間隔一つ分（短いウィンドウでは最大半分）を空けます。
- `statistic_auto_collect_interval` が優先され、未指定なら既存の小表・大表・ヒストグラム間隔を使用します。非正の間隔は従来動作です。行数統計のない外部テーブルは小表のデフォルトを使うため、初回から分散するには正の間隔を明示してください。
- 統計のないテーブルや新しい列も自動収集の予定時刻を待ちます。手動 `ANALYZE TABLE` とクエリ起動の収集は従来どおりで、この予定を進めません。

分散するのは起動時刻であり、CPU・IO コストではありません。各夜の件数は概ね均等になります。実行は変更・健全性判定、チェック周期、処理能力、FE の可用性に依存します。間隔が毎日の閉じた時間より短い場合、次の許可時刻まで一間隔を超えることがあります。間隔またはウィンドウの変更時には将来の予定を再計算します。
