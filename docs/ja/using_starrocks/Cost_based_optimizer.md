...
| enable_statistic_auto_collect_staggered_schedule | BOOLEAN | FALSE | 外部テーブルの自動統計収集を収集間隔内に均等に分散させるかどうか。有効にすると、各 ANALYZE ジョブの初回収集時に次回収集時刻が間隔内でランダムに分散されます。以降の収集は実際の収集時刻から 1 間隔後にスケジュールされ、同じ間隔を共有するジョブは一斉ではなく異なる時点で収集対象になります。間隔はジョブの `statistic_auto_collect_interval` プロパティ（設定時）、未設定の場合はテーブルサイズに応じて `statistic_auto_collect_small_table_interval` または `statistic_auto_collect_large_table_interval` が使われます。 |
...
デフォルトでは、ANALYZE ジョブの次回収集は前回収集の 1 収集間隔後に到来します。同時に収集されたジョブ（例えば 1 つのバッチスクリプトで作成されたもの）は同期したままになり、すべてのジョブが同じ収集ウィンドウで一斉に到来して周期的な収集バーストを引き起こし、間隔の残りの時間はアイドル状態になります。

収集を間隔内に均等に分散させるには、FE 設定項目 `enable_statistic_auto_collect_staggered_schedule` を `true` に設定します（動的パラメータ、デフォルト `false`）。スケジューリングは次のように変わります。

- ジョブの初回収集は通常どおり実行されます。成功後、ジョブの次回収集時刻は 1 収集間隔内のランダムな時点に設定され、同時に収集されたジョブは互いにずれていきます。
- 以降、ジョブはそれぞれの予定時刻に間隔ごとに 1 回チェックされます。
  - 前回の収集以降にテーブルが変更されていれば統計を収集し、次回チェックは実際の収集時刻の 1 間隔後にスケジュールされます。
  - テーブルが変更されていなければ何も収集せず、次回チェックを 1 間隔後に延期します。
- 間隔はジョブの `statistic_auto_collect_interval` プロパティ（設定時）、未設定の場合はテーブルサイズに応じて `statistic_auto_collect_small_table_interval` または `statistic_auto_collect_large_table_interval` が使われます。有効な間隔が変わった場合、ジョブのスケジュールは一度だけ再ランダム化されます。

注意事項:

- 分散スケジュールは有効な収集間隔が 0 より大きい場合にのみ機能します。`statistic_auto_collect_small_table_interval` のデフォルトは `0` のため、小さいテーブル（行数が `statistic_auto_collect_small_table_rows` 未満）のジョブは間隔を明示的に設定しない限り従来の動作のままです。統計が一度も収集されていない大きいテーブルでは、統計が存在するまで行数が不明なため、ランダム分散は初回ではなく 2 回目の収集後に確立されます。
- ジョブのスケジュール確立後、計画収集の間隔は少なくとも 1 収集間隔空きます。例外は 2 つ：最初の分散収集は前回の収集から 1 間隔未満で行われることがあり（これがランダム分散そのものです）、統計のない列はスケジュール外で直ちに収集されます。
- 統計の鮮度はスケジュールに制約されます。予定チェックの直後に発生したデータ変更は次回チェックまで収集されないため、最悪の場合、統計はデータより最大 2 間隔遅れます。
- このオプションを有効にした後の最初のサイクルは従来のスケジュールで収集されます（最後の同期バースト）。分散は各ジョブが新しいスケジュールで初回収集を完了した後に有効になります。
- このスケジュールは特定のテーブルを対象とする ANALYZE ジョブにのみ適用されます。手動の `ANALYZE TABLE` やクエリトリガーのタスクはこのスケジュールに影響しません。統計のない列は予定時刻を待たずに直ちに収集されます。
