...
| enable_statistic_auto_collect_staggered_schedule | BOOLEAN | FALSE | 将已有和新的自动采集任务按表分散到间隔与每日采集窗口内；适用于内表、外表及数据库级任务。 |
...
设置 `enable_statistic_auto_collect_staggered_schedule=true` 可将自动采集按**表**分散到配置的采集间隔及每日允许的采集窗口内。默认值为 `false`。

例如，间隔为 604800 秒、窗口为 01:00–05:00 时，各表会获得分布在未来七个夜间窗口内的不同时段。已有任务和新任务均适用，包括内表、外表以及覆盖整个数据库或所有数据库的任务，无需重建已有任务。

- 第一次调度检查在采集之前分配时段。周间隔的首次时段在未来一周内，不会在原到期时间后再追加一周。开启选项后的第一次检查即使在白天也会初始化已有任务，但实际采集仍只在允许窗口内执行。
- 时段由任务和表的稳定标识及间隔确定。采集成功或检查发现无需采集后，进入下一个日历时段，而非从完成时间再加一个间隔，避免任务重新聚集。
- 元数据回放保留未来时段。重启后已错过的时段或已关闭窗口内的过期任务重新安排到未来时段，不会在下一次窗口开启时集中补跑。周间隔的补排时段在未来一周内。失败的采集可在同一开放窗口重试。
- 使用与自动采集器相同的时区和每日窗口，支持跨午夜窗口。关闭前预留一个检查间隔（短窗口最多预留一半），减少无法及时执行的末尾时段。
- 优先使用任务属性 `statistic_auto_collect_interval`；否则使用现有小表、大表或直方图间隔。非正间隔保持原行为。无行数统计的外表沿用小表默认间隔；若需要分散首次采集，应显式设置正间隔。
- 尚无统计信息的表或新列也等待自动时段。手动 `ANALYZE TABLE` 和查询触发的采集保持原行为，不推进此调度。

分散的是任务启动时间，并非预估的 CPU 或 IO 成本，每晚任务数量近似均衡。实际执行仍受数据变化、统计健康度、检查频率、系统容量和 FE 可用性影响。若间隔短于每日关闭时段，下一个允许时段可能超过一个间隔。修改间隔或窗口会重新计算未来时段。
